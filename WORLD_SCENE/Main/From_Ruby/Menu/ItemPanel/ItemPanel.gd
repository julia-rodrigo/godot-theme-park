...
onready var item_selected_return_cost = item_tags.get_node("cost")
onready var item_selected_description = item_tags.get_node("description")
onready var item_selected_delete_button = item_tags.get_node("delete")

onready var viewport_selected_item = $outerMargin/innerVbox/ContentMargin/SeparateMargin/Body/ScrollContainer/ItemList/Item1/inner/Content/Panel/Viewport


func _process(_delta):
	if Player.selected_item != null:
		item_selected_name.text = Player.selected_item.name
		item_selected_return_cost.text = "Selling Cost: $%d.00" % Player.selected_item.selling_cost
		item_selected_description.text = Player.selected_item.description
		
		if viewport_selected_item.get_children()[0].name != Player.selected_item.sprite._bundled.names[0]:
			print(viewport_selected_item.get_children()[0].name )
			print(Player.selected_item.sprite._bundled.names[0])
			
			viewport_selected_item.get_children()[0].queue_free()
			viewport_selected_item.add_child((Player.selected_item.sprite).instance())
		...
		item_selected_return_cost.text = "Selling Cost: $--.--"
